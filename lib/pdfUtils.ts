...
// Keep the existing dashboard summary function
export async function generateDashboardSummaryPDF({
  ...
  activityFeed,
  user,
  options = { includeCoverPage: false, addWatermark: false },
}: {
  ...
  user: any;
  options?: { includeCoverPage: boolean; addWatermark: boolean };
}) {
  const doc = new jsPDF({ orientation: 'p', unit: 'mm', format: 'a4' });
  const pageHeight = doc.internal.pageSize.getHeight();
  const pageWidth = doc.internal.pageSize.getWidth();
  ...
  let qrCodeDataUrl: string | undefined;
  try {
    qrCodeDataUrl = await QRCode.toDataURL(window.location.origin, { width: 30 });
  } catch (err) {
    console.error('Could not generate QR code', err);
  }
  ...
  if (options.includeCoverPage) {
    ...
    doc.setFontSize(24);
    doc.setTextColor('#14b8a6');
    doc.text('Dashboard Summary Report', pageWidth / 2, 110, { align: 'center' });
    
    doc.setFontSize(12);
    doc.setTextColor(100);
    doc.text(`Generated: ${new Date().toLocaleString()}`, pageWidth / 2, 120, { align: 'center' });
    doc.text(`By: ${user?.user_metadata.full_name || user?.email || 'N/A'}`, pageWidth / 2, 128, { align: 'center' });
  ...
  // --- Header ---
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', 14, startY - 8, 15, 15);
  }
  doc.setFontSize(18);
  doc.setTextColor('#14b8a6');
  doc.text('Dashboard Summary', 35, startY);
  startY = checkPageBreak(startY + 8);
  ...
  doc.setFontSize(8);
  doc.setTextColor(150);
  doc.text(`Filters: ${filterText || 'None'}`, 14, startY);
  startY = checkPageBreak(startY + 10);

  // --- Key Metrics Grid ---
  const kpiBody = [
    [
      { content: `Total Open Orders: ${metrics.totalOrders || 'N/A'}`, styles: { halign: 'center' } },
      { content: `Missing Docs: ${metrics.missingDocs || 'N/A'}`, styles: { halign: 'center' } },
      { content: `Total Denials: ${metrics.totalDenials || 'N/A'}`, styles: { halign: 'center' } },
    ],
    [
      { content: `Docs Complete: ${metrics.docsCompletePercent || 'N/A'}`, styles: { halign: 'center' } },
      { content: `Avg. Days in Stage: ${metrics.avgDaysInStage || 'N/A'}`, styles: { halign: 'center' } },
      { content: `Total Referrals: ${orders.length}`, styles: { halign: 'center' } },
    ],
  ];
  autoTable(doc, {
    startY,
  ...
  // --- Stoplight Status Section ---
  startY = checkPageBreak(startY);
  if (metrics.stoplightCounts) {
    const stoplightBody = [
      ['🟢 Good to Go', metrics.stoplightCounts.green],
      ['🟡 Needs Review', metrics.stoplightCounts.yellow],
      ['🔴 Blocked/Denied', metrics.stoplightCounts.red],
    ];
    autoTable(doc, {
      startY,
      head: [['Stoplight Status', 'Count']],
      body: stoplightBody,
      theme: 'striped',
      headStyles: { fillColor: [75, 85, 99] },
      styles: { fontSize: 8, cellPadding: 2 },
  ...
  doc.setFontSize(10);
  doc.setTextColor(100);
  doc.text("Workflow Pipeline Distribution:", 14, startY);
  startY += 5;
  ...
  if (total > 0) {
    ...
    let currentX = 14;
    stageCounts.forEach((stage, i) => {
        if (stage.count > 0) {
            const segmentWidth = (stage.count / total) * barWidth;
            doc.setFillColor(stageColors[i % stageColors.length]);
            doc.rect(currentX, startY, segmentWidth, 5, 'F');
  ...
  // --- Regression & Activity ---
  startY = checkPageBreak(startY);
  const totalRegressions = regressionInsights.reduce((sum, item) => sum + item.count, 0);
  const regressionBody = regressionInsights.map(item => [item.reason, `${item.count} (${totalRegressions > 0 ? ((item.count / totalRegressions) * 100).toFixed(0) : 0}%)`]);
  
  autoTable(doc, {
    startY,
    head: [['Top Regression Reasons', 'Count']],
    body: regressionBody.length > 0 ? regressionBody : [['No regressions in this period', '']],
    theme: 'striped',
    headStyles: { fillColor: [229, 80, 57] }, // Red
    styles: { fontSize: 8, cellPadding: 2 },
    columnStyles: { 1: { halign: 'right' } },
    tableWidth: (pageWidth / 2) - 20,
    margin: { left: 14 }
  ...
  const activityBody = activityFeed.slice(0, 5).map(item => [item.patient_name, item.content]);
  autoTable(doc, {
    startY,
    head: [['Recent Activity', 'Action']],
    body: activityBody.length > 0 ? activityBody : [['No recent activity', '']],
    theme: 'striped',
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    if (qrCodeDataUrl && i === pageCount) {
      doc.addImage(qrCodeDataUrl, 'PNG', 14, pageHeight - 20, 10, 10);
      doc.text('Live Dashboard', 25, pageHeight - 15);
    }
  }

  const timestamp = new Date().toISOString().split('T')[0];
  doc.save(`PatientSync_Dashboard_Summary_${timestamp}.pdf`);
}
