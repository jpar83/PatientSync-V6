...
import type { Patient, Order, WorkflowHistoryEntry, AuditLogEntry, WorkflowStage, Denial, Equipment } from '../lib/types';
...
export default function PatientDrawerAdaptive({ patientId, open, onClose, onUpdate, startInEditMode = false }: { patientId: string | null, open: boolean, onClose: () => void, onUpdate: () => void, startInEditMode?: boolean }) {
  ...
  const { addNote } = useNoteMutations(patientId || '');
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
        ...
        let historyData: WorkflowHistoryEntry[] = [];
        let auditLogData: AuditLogEntry[] = [];
        let equipmentData: Equipment[] = [];
        let denialsData: Denial[] = [];

        if (orderData || patientData) {
            const [historyRes, auditRes, equipmentRes, denialsRes] = await Promise.all([
                orderData ? supabase.from('workflow_history').select('*').eq('order_id', orderData.id).order('changed_at', { ascending: false }) : Promise.resolve({ data: [], error: null }),
                supabase.from('audit_log').select('*').contains('details', { patient_id: patientId }).order('timestamp', { ascending: false }),
                orderData ? supabase.from('equipment').select('*').eq('order_id', orderData.id) : Promise.resolve({ data: [], error: null }),
                orderData ? supabase.from('denials').select('*').eq('order_id', orderData.id) : Promise.resolve({ data: [], error: null }),
            ]);
            historyData = historyRes.data || [];
            auditLogData = auditRes.data || [];
  ...
  }, [open, startInEditMode, patientId]);
  ...
  const handleStageSave = async ({ newStage, note, regressionReason }: { newStage: WorkflowStage; note: string; regressionReason?: string }) => {
    ...
    await supabase.from('workflow_history').insert({ order_id: order.id, previous_stage: order.workflow_stage, new_stage: newStage, note: note, changed_by: user?.email || 'System' });
    ...
    const { error } = await supabase.from('orders').update({ workflow_stage: newStage, last_stage_note: note, last_stage_change: new Date().toISOString() }).eq('id', order.id);
    if (error) { 
        toast('Failed to update stage.', 'err');
    } else { 
      await writeAuditLog(isRegression ? 'regression' : 'stage_change', {
          changed_user: patient.name,
          patient_id: patient.id,
          details: {
            from: order.workflow_stage,
            to: newStage,
            note,
            reason: regressionReason,
          }
  ...
  const handleArchiveConfirm = async () => {
    ...
    try {
        ...
        await writeAuditLog(newStatus ? 'order_archived' : 'order_restored', { changed_user: patient?.name });
  ...
  const handleExportSnapshot = async () => {
    ...
    try {
        await generatePatientSnapshotPDF({ order, auditLog, denials, user });
              ...
              <AnimatePresence mode="wait">
                {isEditMode ? (
                  ...
                  <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    {activeTab === 'workflow' && <WorkflowHub order={order} patient={patient!} history={history} auditLog={auditLog} onStageChangeClick={() => setShowStageModal(true)} onAddNoteClick={() => setShowNoteModal(true)} onUpdate={onUpdate} />}
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
